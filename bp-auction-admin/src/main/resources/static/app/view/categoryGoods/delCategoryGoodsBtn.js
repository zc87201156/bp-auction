Ext.define("WF.view.categoryGoods.delCategoryGoodsBtn", {
    extend: "Ext.Button",
    alias: "delCategoryGoodsBtn",
    text : '删除',
    ...
    listeners : {
        click : function() {
           ...
           Ext.MessageBox.confirm('警告', '确定删除 ' + records[0].data.id + ' ?', function(btn) {
               if (btn == 'yes') {
                   callapi('auction/admin/categoryGoods/delete', records[0].data, function(result) {
                       if (result.success) {
                           Ext.MessageBox.show({
                               title: '提示',
                               msg: '删除成功',
